...
12. A column-level instance of this pattern occurs when a cached or shared analytical result (e.g., an aggregate query output) is reused by a requester with lawful access to the same underlying tables and columns but not to the specific derivation that produced the value — naive tag- or role-based retrieval gating cannot distinguish these cases because it does not track the aggregation operator or full derivation graph behind a cached value. One evaluation of derivation-gated retrieval — verifying a requester's authorization against every column and operator in a value's derivation lineage rather than its cached tags alone — measured 18.8–25.5% cross-department leakage under naive role/tag-based gating (synthetic and TPC-H benchmarks) versus 0% under derivation-gated retrieval, while preserving 81.5–82.6% of memory/cache reuse [61].
...
61. Lineage-Aware Memory Governance: A Derivation-Gated Framework for Privacy-Preserving Column-Level Access Control in Enterprise AI Agents: Sangaraju & Vissa, IEEE Access, 2026 (DOI: 10.1109/ACCESS.2026.3730363)
